...
filenames = ['run16_vals_542_794_266_497','run16_hues_542_794_266_497','briggs_rauscher_run16_700c_0_5e_86p_1a_0i_DCLS']
colors = [cmap(1), cmap(10), cmap(8),cmap(5)]
legend_names = ['HSV Value','HSV Hue', 'Iodine DCLS','Base DCLS' ]
...
for ax in [ax1, ax2]:  # Apply the same plots to both axes
    ax.plot(time_s, val_s / 255, label=legend_names[0], linewidth=2.5, color=colors[0])
    ax.fill_between(time_s, val_min_s / 255, val_max_s / 255, alpha=0.1, color=colors[0], edgecolor='none')
    
    ax.plot(time_s, hue_s / 255, label=legend_names[1], linewidth=2.5, color=colors[3])
    ax.fill_between(time_s, hue_min_s / 255, hue_max_s / 255, alpha=0.1, color=colors[3], edgecolor='none')
...
ax1.set_xlim(0,440)
ax1.set_yticks([])
ax1.set_xticklabels(np.arange(0, 450, 50), fontsize=15)
ax1.legend(fontsize=15, loc='lower left',facecolor='white',framealpha=1)

# Customize ax2 (Zoomed-in plot)
ax2.set_xlim(100, 140)  # Set the zoom-in range
ax2.set_yticks([])  # Remove y-ticks to keep it clean
# Set the tick positions (every 20 units between 200 and 240)
ax2.set_xticks(np.arange(100, 141, 10))

# Set the corresponding labels for the ticks (same as the tick positions)
ax2.set_xticklabels(np.arange(100, 141, 10), fontsize=15)

# Save figures
fig.savefig('7_figures/HSV_DCLS_starch_plot_BR_color_comp.png', dpi=700)
